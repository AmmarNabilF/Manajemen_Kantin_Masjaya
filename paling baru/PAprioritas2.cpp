...
void boyerMooreSearchMenu(dataUser* currentUser) {
    ...
    if (result) {
        cout << "===============================\n"
             << "Menu ditemukan!\n"
             << "Kode menu: " << result->kodeMenu << "\n"
             << "Nama menu: " << result->namaMenu << "\n"
...
void checkout(dataUser* pembeli) {
    ...
    if (pilihan == 1) {
        // Hapus semua pesanan dari histori
        while (pembeli->stackHistori) {
            histori* temp = pembeli->stackHistori;
            pembeli->stackHistori = pembeli->stackHistori->next;
            delete temp;
        }
            ...
            while (hist) {
                if (hist->idPesanan == curr->idPesanan) {
                    ...
                    if (hist->status == "Complete") {
                        if (prevHist) {
                            prevHist->next = hist->next;
                        } else {
                            userIter->stackHistori = hist->next;
                        }
                        delete hist;
                        break;
                    }
